 ...
 * Creates particles that burst upwards from the bottom corners and fall gracefully.
 */
export const triggerConfetti = () => {
  ...
  // Set canvas resolution based on DPR for sharpness
  ...
  // Helper to spawn a burst of particles from a specific point
  ...
  // Launch from bottom left (angle: 45 degrees approx)
  createConfettiPart(0, [0.15 * Math.PI, 0.35 * Math.PI]);
  // Launch from bottom right (angle: 135 degrees approx)
  ...
  const update = () => {
    ...
    particles.forEach((p) => {
      ...
      if (p.opacity > 0 && p.y <= window.innerHeight + 20) {
        ...
        // Draw small rectangles for confetti flakes
  ...
  // Clean up event listener or canvas on hot-reload/navigation
  return () => {
    cancelAnimationFrame(animationFrameId);
    canvas.remove();
  };
